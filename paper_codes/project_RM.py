...
xls = pd.ExcelFile(data_dir + 'Russell_Sim_RERM-20DMUs.xlsx') # Russell_Sim_Trial1-ERM
#
...
for item in range(1, num_input+1):
    if xdata1 ==[]:
        xdata1 = np.array(list(pre_xdata.iloc[1:,item + 0]) )
        xdata1 = np.vstack((xdata1, list(pre_xdata.iloc[1:,item  + num_input + 1])) )
    else:
        xdata1 = np.vstack((xdata1,  np.array(list(pre_xdata.iloc[1:,item + 0]) ) ) )
        xdata1 = np.vstack((xdata1,  list(pre_xdata.iloc[1:, item  + + num_input + 1] ) ) )
...
#
gama = np.array( list( data_real['Gamma'] ), dtype = np.float64 )
# xdata1 = np.array(xdata1, dtype = np.float64).transpose(1,0)

columns = list(lambdas.columns[2:2 + num_lambdas])
lambdas1 = []
for item in range(len(columns)):
    if lambdas1 == []:
        lambdas1 = np.array(lambdas.iloc[:num_lambda_rows, item + 2]) # , dtype = np.float64
    else:
        lambdas1 = np.vstack( (lambdas1, np.array( lambdas.iloc[:num_lambda_rows, item + 2]))) # , dtype = np.float64)
#
...
def check_reanking(produced, truth):
    ...
    produced = np.array(produced, dtype = np.float64)
    truth = np.array(truth, dtype = np.float64)[:,1:]
...
def tfunc(lamba, num_simul, xdata1, data_real):
    ...
    for gam_iter in range(0, gama.shape[0]): # iteration for gamma
        ...
        # x_dim = xdata1.shape[1]//4 # [x_l, x_u], [y_l, y_u]
        #
        ...
        for k in range(num_simul):
            ...
            for idmu in range(dmu_size): # iteration for DMU
                ...
                #
                gmdm_ind = lamba.shape[0] * (gam_iter - 0) + idmu
                #
                # print('iteration over lambda: ', gmdm_ind)
                # -------------
                # for idd in range(lamba.shape[0]):
                #     uy += np.array([ (x[idd,i] * lamba[idd, gmdm_ind ]) / x[idmu, i]  for i in range(x_dim) ]).sum()
                #     vx += np.array([ (y[idd,i] * lamba[idd, gmdm_ind ]) / y[idmu, i]  for i in range(x_dim) ]).sum()
                uy = np.array([( np.dot( X[:,i,k] , lamba[:, gmdm_ind ]) / X[idmu, i, k] ) for i in range(num_input)], dtype = np.float64).sum()
                vx = np.array([ 1./( np.dot( Y[:,i, k] , lamba[:, gmdm_ind ]) / Y[idmu, i, k] ) for i in range(num_output)], dtype = np.float64).sum()
                #
                target = (uy + vx)/(X.shape[1] + X.shape[1])
                # print('uy: {}  and  vx: {} '.format( uy, vx))
                # target = 1  if uy/vx > 1 else (1./(X.shape[1] + X.shape[1]) ) * (uy + vx)
                #
                dmus.append(target)
            #
            sim_dmu.append(dmus)
    ...
    gama_dmu = np.array(gama_dmu, dtype = np.float64).transpose(1,0,2)
...
sim_res = np.array(sim_result, dtype = np.float64).transpose(2,1,0) # DMU X GAMMA X simulations
...
pd.DataFrame(final_res).to_csv("results/final_Russell_Sim_Trial1-RM-New" + str(num_simul) + ".csv")
average = final_res.sum(axis=1)/final_res.shape[1]
pd.DataFrame(average).to_csv("results/average_Russell_Sim_Trial1-RM-New" + str(num_simul) + ".csv")
